...
import sys
sys.path.append('../..')
from NaiveMC.mcmc import hbm_joint
...
def data_given_local( local, mass_ob, mass_err, rad_ob, rad_err ):
	...
	total_loglikelihood = 0. - 0.5* np.sum( (mass_ob - mass_th)**2./mass_err**2. ) \
						 - 0.5* np.sum( (rad_ob - rad_th)**2./rad_err**2. )
...
def local_given_hyper(hyper,local):
	...
	total_loglikelihood = 0. - 0.5* n_group * np.log(sm) - 0.5* np.sum( (mass_th-mu)**2./sm**2. )
...
def hyper_prior(hyper):
	...
	total_loglikelihood = 0. - np.log(c) - 0.5* (gamma-1.)**2. - np.log(sm)
...
plt.savefig('test_angie_plain.png')


np.savetxt('test_angie_plain.out', hyper_chain[:stop_step,:])
