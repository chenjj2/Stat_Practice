...
'''
prestep: straight_vary.py
same setup, but
1. fix temperature=1, stepscale=2
2. vary random seed
3. run from the last step of previous run
'''
...
parser.add_argument("hyper_prob_input")
parser.add_argument("local_prob_input")
...
hyper_prob0 = np.loadtxt(args.hyper_prob_input)
hyper_stepsize = stepsize * np.ones(3*n_pop)
local_prob0 = np.loadtxt(args.local_prob_input)
local_stepsize = stepsize * np.ones(n_fixm + 2*n_varm)
...
### save
np.savetxt(args.output+'_hyper.out',hyper_chain[:stop_step,:])
np.savetxt(args.output+'_loglike.out',loglike_chain[:stop_step])
np.savetxt(args.output+'_repeat.out',repeat_chain[:stop_step])

### save last
last_ind = stop_step-1
np.savetxt(args.output+'_hyper_prob_last.out',hyper_prob_chain[last_ind,:])
np.savetxt(args.output+'_local_prob_last.out',local_prob_chain[last_ind,:])
